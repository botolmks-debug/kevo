...
STEP 3 - This is the FINAL "payoff" slide of the story: title "${lastSlide.title}", text "${lastSlide.desc}". Compose the scene so it naturally supports this closing moment. Frame it like a proper PRODUCT shot (appropriate product-to-frame scale, e.g. tabletop/surface/hand-held framing) — do NOT reuse a full-body human framing/scale from a people-scene just because IMAGE 2 happened to feature a person; the product should look naturally sized for a product photo, not oddly blown up to fill a person-sized frame.
...
LANGKAH 3 - Ini slide PENUTUP/puncak cerita: judul "${lastSlide.title}", teks "${lastSlide.desc}". Susun adegannya supaya natural mendukung momen penutup ini. Bingkai foto ini sebagai foto PRODUK yang proporsional (mis. di atas meja/permukaan/dipegang tangan) — JANGAN ikut memakai framing/skala tubuh manusia dari IMAGE 2 hanya karena kebetulan IMAGE 2 menampilkan orang; produknya harus terlihat berukuran wajar untuk foto produk, bukan dibesarkan aneh sampai memenuhi bingkai seukuran orang.
