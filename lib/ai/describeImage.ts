...
export async function describeProductImage(input: {
  ...
  const instruction =
    (input.lang ?? "id") === "en"
      ? `${industryLineEn}Look at this product photo. In ONE or TWO short sentences, describe the MAIN product a small business would sell here: what it is, its type/variant, color/packaging, and the brand name ONLY if clearly legible. If the photo shows a person, focus ONLY on a physical product they are holding, using, or that is applying/touching their body (e.g. a skincare bottle, a food item, a tool) — NEVER describe their clothing, hijab, jewelry, accessories, or outfit as the product, UNLESS the stated industry is fashion/apparel/clothing. If no clear physical product is visible near the person, say so plainly instead of guessing from what they're wearing. Reply with the description only — no preamble, no quotes.`
      : `${industryLineId}Lihat foto produk ini. Dalam SATU atau DUA kalimat pendek, jelaskan PRODUK UTAMA yang dijual sebuah usaha kecil di sini: apa produknya, jenis/varian, warna/kemasan, dan nama merek HANYA kalau jelas terbaca. Kalau foto menampilkan orang, fokus HANYA ke produk fisik yang dipegang, dipakai/dioleskan ke tubuh, atau sedang dipraktikkan (mis. botol skincare, makanan, alat) — JANGAN PERNAH menganggap baju, hijab, perhiasan, atau aksesoris yang dikenakan model sebagai produknya, KECUALI industri yang disebutkan memang fashion/pakaian. Kalau tidak ada produk fisik yang jelas di dekat orang itu, katakan saja tidak jelas — jangan menebak dari apa yang dia kenakan. Jawab deskripsinya saja — tanpa basa-basi, tanpa tanda kutip.`;
