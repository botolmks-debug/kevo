...
export function StandarContent({
  ...
  const canGenerateAI =
    !!selectedImage && isAiImage && judul.trim().length > 0 && descriptions.some((d) => d.trim().length > 0);
  ...
  function buildTextValues(): Record<string, string> {
    const nv: Record<string, string> = { title: judul };
  ...
  function handleUseOriginal() {
    ...
    setValues({ ...buildTextValues(), photo: selectedImage.publicUrl });
  ...
  async function handleGenerateAI() {
    ...
    try {
      ...
      setPhoto(d.dataUri);
      setValues({ ...buildTextValues(), photo: d.dataUri });
      setGenStatus("success");
    } catch (e) {
      setGenStatus("error");
  ...
  async function handleSimpanPng() {
    ...
    try {
      ...
      const saved = await saveManualContent({
        ...
        onImageText: values.title ?? judul,
        caption,
        ratio,
        jenis: "produk",
        existingId: savedId,
